...
> 補足（2026-06-17 handoff）: Kaggle push handoff の **SSOT は `docs/next_plan.md`**。本書には live-smoke の `rc=0` / candidate path / passed 件数のような揮発情報を重複保持しない。現行の push readiness は `probe_author` を **未検証 backstop** として advisory 扱いにしており、特に **token auth + 初回 push(header-only probe)** では owner を完全には保証しない。この限界は push 判断時に別途 human review（slug / owner / license basis、特に **Web UI での slug / owner 最終確認は必須 gate**）で埋める前提。
...
## 7. 状態
- 2026-06-15 計画確定（branch `feat/lm-recurrent`）。実装は llterm へ委譲可（本 doc を grounding に自走）。本書 = **LM recurrent の設計/計画**の正本。Kaggle push handoff の進捗状態は `docs/next_plan.md` を正本とする。
...
- 2026-06-16 memory telemetry 追記:
  ...
  - 2026-06-16〜2026-06-17 追記: Kaggle offload 用の builder / preflight / prepare / readiness はこのブランチで実装済み。ここでは**設計と契約だけ**を残し、candidate path / passed 件数 / live-smoke の `rc` / GPU quota CLI 不具合の詳細などの**揮発情報は `docs/next_plan.md` を正本**とする。恒久的に言える点だけを書くと、(a) builder は private + internet disabled + GPU disabled を safe default にした deterministic bundle 生成、(b) preflight は manifest / metadata / copied_files / runner sidecar を fail-closed で検証、(c) readiness は publish 自体を行わず local preflight + limited live Kaggle(auth/quota) を quoted push command 提示までまとめる、(d) **どこまで live 確認が通ったかという時点依存の状態は `docs/next_plan.md` のみを正本**とし、本書では複写しない、である。現行の人間ゲート条件（license basis / bundle wording / Web UI slug-owner 確認）は `docs/next_plan.md` に従う。
