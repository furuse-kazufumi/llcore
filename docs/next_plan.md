...
> 2. dataset-backed 正本 candidate `D:\projects\llcore_kaggle_livecheck_20260617f` の内容を確認する。特に root `.kaggleignore` が `dataset_payload/` を除外していること、`dataset_payload/` 配下の `config.json` / `input_corpus.txt` / `src/llcore` / `llcore` / `dataset-metadata.json` が揃うこと、root `kernel-metadata.json.dataset_sources=["furusekazufumi/llcore-lm-compare-support"]` が一致していることを見る。
> 3. `D:\projects\llcore_kaggle_livecheck_20260617d` は **再 push しない停止済み候補**、`D:\projects\llcore_kaggle_livecheck_20260617e` は **初代 dataset-backed だが superseded 済み候補**として保持し、publish を伴う次回差分は `...20260617f` 以降の dataset-backed 系列だけを使う。
> 4. publish を再開する段になったら、先に **正本同一性の human gate** を通す。具体的には `D:\projects\llcore_kaggle_livecheck_20260617f` を保護対象として扱い（削除/改名しない）、`git status --short` が `docs/SESSION_SUMMARY.md` 以外 clean であること、candidate の `bundle_manifest.json` / `dataset_payload_manifest.json` が現物と矛盾していないこと、再実行した `kaggle_push_readiness.py --bundle-dir ...20260617f` が `rc=0` を返すことを確認する。
> 5. その上で `kaggle datasets create -p "D:\projects\llcore_kaggle_livecheck_20260617f\dataset_payload" --dir-mode zip`（初回）または `kaggle datasets version -p "D:\projects\llcore_kaggle_livecheck_20260617f\dataset_payload" --dir-mode zip -m "update dataset payload"`（更新）で dataset を供給し、その後に kernel push を human gate で判断する。**create の場合は Visibility（Private/Public）を人間が明示確定してから実行**する。なお `--dir-mode zip/tar` は「folder skip を回避する」だけで、Kaggle 側で archive が runner から import 可能な展開ディレクトリとして mount されるかは未実証であり、`import llcore` を保証しない点を前提に扱う。
