...
> 2026-06-17 owner gate 追記: kernel push の human gate は **`rc=0` 単独を owner 保証根拠に使わない**。理由は、`credential_sources` に `access_token` / OAuth 系が含まれると `owner_check_status=validated_local_config_token_present` でも **実際の push 主体が local `kaggle.json` と別アカウントである可能性が残る**ためである。したがって gate は **既存 slug 更新** と **初回 create** を分岐する。既存 slug 更新では `(a)` `auth.owner_check_status` が `advisory_` で始まらないことに加え、`(b)` **`auth.owner_verification_passed == true`（同値に `auth.probe_author_status == "validated_against_owner"`）** を必須条件とする。これが欠ける場合は Web UI 上の owner 目視だけでは不十分で、push しない。初回 create では `kaggle kernels list -m --page-size 1 --csv` が header-only となり `probe_author_status=advisory_unverified_empty_probe` / `owner_verification_passed=false` が正常系になり得るため、`(c)` readiness JSON の `probe_row_state == "header_only_or_first_push"` を確認したうえで、**同じ認証状態のまま `kaggle kernels list -m` の live 出力と Web UI を人間が見て「まだ slug が存在しない初回 create」だと確定した場合のみ** create を許容する。現 candidate では historical `...20260617c` の存在があるため、実運用ではまず Web UI で slug 実在を確認し、通常は既存 slug 更新側の stricter gate を使う。
