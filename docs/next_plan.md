...
> 最終更新: 2026-06-17。現作業の正本は **memory harness 1 本 + Kaggle 導線 4 本 + 対応テスト 5 本 + docs**。現物 `git status --short` では `docs/LM_RECURRENT_PLAN.md` / `docs/PROGRESS.md` / `docs/SESSION_SUMMARY.md` / `docs/next_plan.md` / `scripts/memory_footprint_harness.py` が変更中で、`scripts/build_kaggle_lm_compare_bundle.py` / `scripts/kaggle_bundle_preflight.py` / `scripts/prepare_kaggle_lm_compare_bundle.py` / `scripts/kaggle_push_readiness.py` と対応テスト群が未追跡。Kaggle focused gate の最新は **4 ファイル集合で `50 passed`**、memory harness を含む focused gate の最新は **`57 passed`**。現行 `kaggle_push_readiness.py` は push credential (`kaggle.json` または `KAGGLE_USERNAME`+`KAGGLE_KEY`) の存在 + `kaggle kernels list -m --page-size 1 --csv` 疎通で auth を見ており、CPU bundle は quota check を skip、GPU bundle のみ `kaggle quota -v` を使う。live 実測は **CPU bundle の ready path (`rc=0`)** まで確認済みで、GPU bundle も試したが **ローカル Kaggle CLI 2.2.1 の `quota` サブコマンド自体が `not enough values to unpack (expected 2, got 1)` で失敗**したため quota live path は確認不能。実 `kaggle kernels push` は未実施。なお `kaggle quota -v` は週次の GPU 合計時間しか返さないため、green でも **`machine_shape` 別の空き枠**までは保証しない。下の旧 `p1_compare` / staged 2 件前提は**履歴メモ**であり、再開判断には使わない。
...
2. **P1: held-out PPL 改善**（block_size 拡大 / `--config p1` / データ追加）。過学習は dropout で抑える既知教訓を踏襲。
   ...
   - 2026-06-16 追記: `--lengths` は正の整数のみを **fail-closed** で受け、空要素 (`64,,128`) や非整数 (`64,abc`) も reject する。JSON には raw 入力の `config.lengths` と、昇順・重複排除した `lengths_effective` を併記し、後者を実効値として読む。
...
- 2026-06-17 EXIT 追記: 現在の作業木は `scripts/memory_footprint_harness.py` の修正と、Kaggle 導線 4 本（`build_kaggle_lm_compare_bundle.py` / `kaggle_bundle_preflight.py` / `prepare_kaggle_lm_compare_bundle.py` / `kaggle_push_readiness.py`）、対応テスト 5 本、docs 3 枚、hook 更新の `docs/SESSION_SUMMARY.md` が dirty。Kaggle 側の focused は **builder / preflight / prepare / readiness の 4 ファイル集合で `50 passed`**、broad gate は **`262 passed, 401 deselected`**。live 実測は依然 **auth failure path (`rc=3`) のみ**で、quota live path / actual `kaggle kernels push` は未実施。
- 2026-06-17 EXIT 追記: 次の具体的な一手は **新規実装ではなく人間ゲート直前の最終整理**。再開したら、(1) `git status --short` で現 dirty を再確認、(2) Kaggle 3 スクリプト + テスト群を 1 束、memory harness 束を別束として commit 単位を切る、(3) その後に `scripts/prepare_kaggle_lm_compare_bundle.py` / `scripts/kaggle_push_readiness.py` を使って fresh bundle を repo 外へ再生成し、**現行契約どおり CPU ready path が `rc=0` のままか**、必要なら GPU bundle で quota/live path を追加確認する。ここまでは自律で可。`kaggle kernels push` に進む段になったら、その直前で `docs/next_plan.md` を更新してから `⟦LLTERM_CHOICE⟧` に切り替える。
- 2026-06-17 再開追記: 再開後の現物 `git status --short` では `docs/LM_RECURRENT_PLAN.md` / `docs/PROGRESS.md` / `docs/SESSION_SUMMARY.md` / `docs/next_plan.md`、`scripts/memory_footprint_harness.py`、Kaggle 導線 4 本と対応テスト 5 本が dirty だった。前回 `SESSION_SUMMARY` の git status ブロックで `docs/LM_RECURRENT_PLAN.md` が脱落していたため、その点は stale 記録として扱う。commit 束分けの正本はこの現物状態を使う。
...
- 2026-06-17 再開追記: その後の auth 契約修正後に repo 外 fresh bundle `C:\Users\puruy\AppData\Local\Temp\llcore_kaggle_livecheck_20260617b` でも再実測した。manifest hash は前回と同一で、`py -3.11 scripts/kaggle_push_readiness.py --bundle-dir ...` は **CPU ready path `rc=0`** に到達し、`quota_rows=0 quota_checked=cpu` を出した。したがって現行 live で確認済みなのは **CPU bundle の ready path** までで、GPU quota live path と actual `kaggle kernels push` はなお未実施。
