...
> 2026-06-17 human gate 補正: それでも owner / write 権限は機械証明されない。`owner_check_status` non-advisory と `auth.authenticated=true` は **必要条件にすぎず**、`owner_verification_passed=false` / `target_slug_existence="unverified_by_probe"` のままでも成立する。したがって option 1 の load-bearing control は引き続き **Web UI owner 一致確認 + `kaggle kernels get` 後の remote/local 差分確認**であり、`kaggle kernels get` の差分対象は `runner.py` / `kernel-metadata.json` に限定せず、`push_payload.included_files` に含まれる remote 取得可能ファイル全体を基準に解釈する。verify と push の間に bundle を触らないこと、可能なら verify 直後にそのまま push することも TOCTOU 緩和として前提にする。
