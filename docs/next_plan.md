...
- **(済) scripts を strict gate へ allowlist 段階導入(40本到達)** commits `358ac4a`(18)→`abc37aa`(23)→`318a65b`(29)→`e5cea1b`(36)→`6c6cd76`(39)→`2cb9a6d`(40)。`ci/mypy_strict_scripts.txt` に集約、CI を `mypy src $(grep -v '^#' ci/mypy_strict_scripts.txt) --strict` に拡張。現在 **src 67 + scripts 40 = 107ファイルで strict 0**。
  ...
  - **(新軸 2026-06-22) decode 軸レイテンシハーネスを新設** commit `11bdb4a`: アーキ編の第4軸。a7 が prefill/batch-forward 全体を測ったのに対し、`scripts/decode_latency_sweep.py` は **context age T での per-token decode コスト**(streaming 生成 regime)を測る。本スイート実測(n_embd256/L4, T128→2048, repeats7): **recurrent decode = 完全 flat(0.79ms 一定, ×0.995, p≈-0.006 = O(1))** / **GPT decode = ×77.7 爆増(16→1242ms, p≈1.35-1.49, 1024→2048 で ×5.1 = attention O(T²) regime 発現)** / RWKV は 512以降 flat だが T128/256 は startup 外れ値(honest 注記)。RAD 接地済(先行研究=vLLM/KV cache/linear attention は accelerator/serving 寄り、本軸=自宅CPU実機 decode scaling 対比で差別化)。回帰テスト7件、mypy strict/ruff green。記事ネタ + 実数値は ARTICLE_SEEDS に記録。**honest 留保**: この GPT は KV cache 無(production は cache で O(T)/token、ただし cache 有でも増大 vs recurrent O(1) flat の質的対比は不変)。残: 公開時に SVG 化(a7 latency_scaling.svg と同様、human gate 内)。
