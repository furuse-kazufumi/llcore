...
- **(済) scripts を strict gate へ allowlist 段階導入** commit `358ac4a`(18本)+`abc37aa`(23本へ): py.typed 後に individually+batch とも strict-clean な scripts を `ci/mypy_strict_scripts.txt` に集約、CI を `mypy src $(grep -v '^#' ci/mypy_strict_scripts.txt) --strict` に拡張。現在 **src 67 + scripts 23 = 90ファイルで strict 0**。含む: Kaggle bundle pipeline 全体(build/preflight/prepare/readiness)・nas_pareto(±report)・記事裏付け poc(0a/1a/1b/3a)・記事ハーネス・chat_endurance 等の load-bearing tooling。allowlist 不変条件=「消して黙らせず直して追加」。各 commit で複合 strict 0 + ruff + 関連テスト green 実測。
