...
> 最終更新: 2026-06-16 (EXIT 準備のみ。`scripts/p1_compare.py` は legacy `cfg={...}` fallback の新規配線、optional セルの `?` 正規化、`ppl_gate_pass` tri-state、headline 欠損時の `--json` 非巻き添え、nested cfg graceful degradation まで反映し、`tests/unit/test_p1_compare.py` は 12 case / 12 passed。残る人間ゲートは corpus staging 側。llcore 側の現 `git status` では **staged は `scripts/p1_compare.py` と `tests/unit/test_p1_compare.py` の 2 件のみ**で、unstaged/未追跡には `docs/ARTICLE_SEEDS.md` / `docs/LM_P0_PLAN.md` / `docs/LM_RECURRENT_PLAN.md` / `docs/PROGRESS.md` / `docs/SESSION_SUMMARY.md` / `docs/next_plan.md`、`src/llcore/lm/__main__.py` / `data.py` / `trainer.py`、新規 `src/llcore/lm/corpus.py`、新規 `scripts/p1_*` 群、新規 `tests/unit/test_lm_cli.py` / `test_lm_corpus.py` / `test_p1_*` 群が残っている。`docs/SESSION_SUMMARY.md` は Stop hook `raptor-auto-summary` が stop 時点で自動生成した point-in-time snapshot であり、その後の staging 状態とはズレうる。`collect_research_seeds.py` 側では裸の `記事化` / `published` レグを除去済みだが、legacy shorthand `→ #...` / `→ 記事...` はまだ consumed 判定に残る。dirty の実体は `git status` を正とする)
...
1. まず `git status --short` を再確認し、**staged は `scripts/p1_compare.py` + `tests/unit/test_p1_compare.py` の 2 件のみ**、それ以外の docs / `src/llcore/lm/*` / 新規 script/test 群は unstaged/未追跡という前提から再開する
2. `scripts/p1_compare.py` 束を先に扱うなら、追加編集前に `py -3.11 -m pytest tests/unit/test_p1_compare.py -q` / `ruff` / `mypy` の最終再確認を 1 回流し、その後 docs 束とは分けて commit 準備する
3. コード束をこれ以上触らないなら、`★ユーザー判断 (2026-06-16, ccr 経由)` を開き、`verified_safe_learning` publish と `self_evolving_agents` precision rerun 本実行のどちらの人間ゲートを先に回収するかへ戻る
4. rerun 実行が承認された場合のみ、`queries_refined_candidate.txt` の SHA256 `2AB6A443E70D7A58DDDCFFE4213BF0156960C48E89109245CC9C34F74D6B7D73` を再確認して fetch / 再生成へ着手
