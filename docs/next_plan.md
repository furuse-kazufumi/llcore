...
> 2026-06-17 再開追記: その後、監査材料を machine-readable にするため `kaggle_bundle_preflight.py` の report 契約を拡張した。fresh candidate に対する現行 `prepare_report.json` / `preflight_report.json` では、`checks.config.dataset_metadata_path == "dataset_payload/dataset-metadata.json"`、`checks.manifest.dataset_publish_dir == "dataset_payload"`、`checks.manifest.publish_safety.status == "passed"`、`checks.manifest.publish_safety.scanned_text_files` に `config.json` / `dataset-metadata.json` / `dataset_payload_manifest.json` / `input_corpus.txt` / `LICENSE` / `NOTICE` が並び、`scanned_archive_text_member_count == 102` を確認できる。これにより **「bundle root から入った preflight が実際に dataset publish target を検証し、archive 内テキスト member まで secret/path scan した」**ことを JSON 単体で読めるようになった。対応 focused gate は `py -3.11 -m pytest tests/unit/test_kaggle_bundle_preflight.py tests/unit/test_prepare_kaggle_lm_compare_bundle.py -q` で **`48 passed`**、4 ファイル focused gate は再度 **`112 passed`**。
