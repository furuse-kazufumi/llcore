...
> 2026-06-17 owner gate 追記: kernel push の human gate は **`rc=0` 単独を owner 保証根拠に使わない**。理由は、`credential_sources` に `access_token` / OAuth 系が含まれると `owner_check_status=validated_local_config_token_present` でも **実際の push 主体が local `kaggle.json` と別アカウントである可能性が残る**ためである。したがって gate は **既存 slug 更新** と **初回 create** を分岐するが、その分岐は **`probe_row_state` の値だけでは決めない**。`probe_row_state=existing_slug_seen` は「認証中ユーザーに kernel が 1 件以上ある」、`header_only_or_first_push` は「`kaggle kernels list -m` が header-only だった」ことしか意味しないため、対象 slug `furusekazufumi/llcore-lm-compare` の実在判定には使わない。既存 slug 更新では `(a)` `auth.owner_check_status` が `advisory_` で始まらないことに加え、`(b)` **`auth.owner_verification_passed == true`（同値に `auth.probe_author_status == "validated_against_owner"`）** を必須条件とする。これが欠ける場合は Web UI 上の owner 目視だけでは不十分で、push しない。初回 create は **同じ認証状態のまま** `kaggle kernels list -m` の live 出力と Web UI の両方で対象 slug 不在を人間が確認した場合だけ許容する。`probe_row_state == "header_only_or_first_push"` はその補助情報に留め、create 許可条件そのものにはしない。現 candidate では historical `...20260617c` の存在があるため、実運用ではまず Web UI で slug 実在を確認し、通常は既存 slug 更新側の stricter gate を使う。
...
> 2026-06-17 最新局所 gate 追記: `py -3.11 -m pytest tests/unit/test_kaggle_push_readiness.py -q` は **`46 passed`**、`py -3.11 -m pytest tests/unit/test_build_kaggle_lm_compare_bundle.py tests/unit/test_kaggle_bundle_preflight.py tests/unit/test_prepare_kaggle_lm_compare_bundle.py tests/unit/test_kaggle_push_readiness.py -q` は **`126 passed`**（warning 2 件は duplicate zip member 系の既知テスト由来）、`py -3.11 -m ruff check scripts/kaggle_push_readiness.py tests/unit/test_kaggle_push_readiness.py` と `MYPYPATH=D:\projects\llcore\src py -3.11 -m mypy scripts/kaggle_push_readiness.py tests/unit/test_kaggle_push_readiness.py` も通過した。
