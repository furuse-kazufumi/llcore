...
> 2. dataset-backed 正本 candidate `D:\projects\llcore_kaggle_livecheck_20260617e` の内容を確認する。特に `dataset_payload/` 配下の `config.json` / `input_corpus.txt` / `src/llcore` / `llcore` / `dataset-metadata.json` と、root `kernel-metadata.json.dataset_sources=["furusekazufumi/llcore-lm-compare-support"]` が揃っていることを見る。
> 3. `D:\projects\llcore_kaggle_livecheck_20260617d` は **再 push しない停止済み候補**として保持し、publish を伴う次回差分は `...20260617e` 以降の dataset-backed 系列だけを使う。
> 4. publish を再開する段になったら、先に `kaggle datasets create -p "D:\projects\llcore_kaggle_livecheck_20260617e\dataset_payload"`（または version）で dataset を供給し、その後に kernel push を human gate で判断する。
