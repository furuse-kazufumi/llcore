...
> 4. publish を再開する段になったら、先に `kaggle datasets create -p "D:\projects\llcore_kaggle_livecheck_20260617f\dataset_payload"`（初回）または `kaggle datasets version -p "D:\projects\llcore_kaggle_livecheck_20260617f\dataset_payload" -m "update dataset payload"`（更新）で dataset を供給し、その後に kernel push を human gate で判断する。
> 5. GPU bundle を後続で扱う場合のみ、Web UI quota 確認または CLI 更新 / 別環境での quota 再確認を先に行う。
...
> 2026-06-17 人間判断追記: dataset-backed 正本候補 `D:\projects\llcore_kaggle_livecheck_20260617f` については、`kaggle datasets create` / `kaggle datasets version` / `kaggle kernels push` のいずれも **この時点では実行しない** 判断を受領した。したがって `...20260617f` は **local smoke / readiness / focused gate 完了済みの publish 待機候補**として保持し、次回はこの bundle を正本として再開する。現時点で未実施の外部操作は `dataset_payload` の publish（create または version）と、その後の kernel push である。
