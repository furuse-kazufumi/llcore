...
> 2026-06-17 owner gate 追記: kernel push の human gate は **`rc=0` 単独を owner 保証根拠に使わない**。最低条件は `(a)` `auth.owner_check_status` が `advisory_` で始まらないこと、`(b)` readiness stderr に `warning:` が無いか、または warning が `probe_author_status=advisory_unverified_empty_probe` のみで **Web UI 上 slug 不存在を初回 create として確認済み**であること、`(c)` Web UI `https://www.kaggle.com/code/furusekazufumi/llcore-lm-compare` で slug 実在を見た場合は owner も `furusekazufumi` と一致すること、の 3 点に固定する。したがって **slug が既に存在する場合だけ owner 上書き防止チェックを厳格適用し、slug が無い場合は初回 create として許容**する。現 candidate では historical `...20260617c` の存在があるため、実運用ではまず Web UI で slug 実在を確認する。
