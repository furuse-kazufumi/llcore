...
> 最終更新: 2026-06-17。現作業の正本は **memory harness 1 本 + Kaggle 導線 4 本 + 対応テスト 5 本 + docs**。この冒頭は**時点メモ**であり、status の正本は都度 `git status --short` を参照する。本 docs 更新を commit した直後は clean だったが、**編集中は少なくとも本ファイルが dirty** になり得る。直近 commit 列挙は handoff 用の参考値に留める: `514f2c2`（handoff docs 追記）, `4bd61e2`（Kaggle bundle guards + honest-disclosure + memory harness ordering）, `90378cf`（session handoff 更新）, `1d1234a`（Kaggle readiness tooling）。Kaggle focused gate の最新は **4 ファイル集合で `50 passed`**、memory harness を含む focused gate の最新は **`57 passed`**、追加 hardening の局所 gate (`test_memory_footprint_harness.py` / `test_build_kaggle_lm_compare_bundle.py` / `test_kaggle_push_readiness.py`) は **`31 passed`**、対応 `mypy` / `ruff` も通過。現行 `kaggle_push_readiness.py` は push credential (`kaggle.json` または `KAGGLE_USERNAME`+`KAGGLE_KEY`) の存在 + `kaggle kernels list -m --page-size 1 --csv` 疎通で auth を見ており、CPU bundle は quota check を skip、GPU bundle のみ `kaggle quota -v` を使う。live 実測は **CPU bundle の ready path (`rc=0`)** まで確認済みで、GPU bundle も試したが **ローカル Kaggle CLI 2.2.1 の `quota` サブコマンド自体が `not enough values to unpack (expected 2, got 1)` で失敗**したため quota live path は確認不能。実 `kaggle kernels push` は未実施。なお `kaggle quota -v` は週次の GPU 合計時間しか返さず、readiness green でも **`machine_shape` 別の空き枠**までは保証しない。代替導線としては、Kaggle Web UI で quota を手動確認するか、別環境 / 別 CLI version で `kaggle quota -v` 疎通を再試行する。下の旧 `p1_compare` / staged 2 件前提は**履歴メモ**であり、再開判断には使わない。
