...
> 2026-06-17 fresh evidence 追記: push 直前の 3 系統証跡は current code で再生成済み。`prepare_report.json` は **2026-06-17 15:43:10**、`preflight_report.json` は **15:39:32**、`readiness_run_runner.json` は **15:43:18**。これで少なくとも current candidate `D:\projects\llcore_kaggle_livecheck_20260617g` の bundle / preflight / readiness は同じコード世代へ揃っている。なお gate の目的は「意図しない既存 kernel への新バージョン投入を防ぎ、owner / slug 誤認を人間確認で閉じること」であり、`kaggle kernels push` 自体は対象が存在すれば update、無ければ create、他人所有なら 403 に倒れる。したがって存在ゲートは **安全向上の補助**であって、Web UI/CLI の確認後も TOCTOU は理論上残る。
