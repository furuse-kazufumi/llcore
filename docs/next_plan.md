...
> 2026-06-17 human gate 補正: option 1 を使う場合、owner 一致の**主たる機械条件**は `auth.probe_author_status == "owner_slug_matches_authenticated_user"` とする。`owner_check_status` non-advisory と `auth.authenticated=true` は補助的な必要条件にすぎず、`owner_verification_passed` は現実装で常に `false` のため根拠に使わない。`kaggle kernels get` 成功や Web UI owner 表示は read 可否 / 表示上の owner を示すだけで、認証主体と push 先 owner の一致や write 権限の正証明にはならないため、**補助確認**に格下げする。さらに `kaggle kernels get` 後の差分確認対象は remote で取得できる **kernel source script/notebook と `kernel-metadata.json`** に限定し、`src_llcore.zip` / `pkg_llcore.zip` / `input_corpus.txt` / `config.json` 等の dataset payload 側ファイルは `kernels get` では取得できないので差分対象に含めない。verify と push の間は bundle を一切変更せず、**verify を最後のローカル操作にして直後に push** する。補償制御を採らない場合の安全側デフォルトは option 2 であり、option 1 は通常許容フローではなく human gate 付きの例外経路として扱う。
> 2026-06-17 owner gate 再補正: 上記の `auth.probe_author_status == "owner_slug_matches_authenticated_user"` を **hard gate には使わない**。理由は current evidence が `advisory_unverified_empty_probe` / `target_slug_existence="unverified_by_probe"` を返しており、probe は対象 slug の存在や ownership を機械証明しないためである。したがって option 1 の自動 gate は `owner_check_status` non-advisory と `auth.authenticated=true` を **必要条件**として見るに留め、owner / write 権限の load-bearing control は **Web UI owner 一致確認 + `kaggle kernels get` を使った補助確認**へ戻す。初回 create 分岐もこの前提でのみ意味を持ち、probe 系フィールドだけで create/update を決めない。現時点の推奨は引き続き option 2（停止）である。
