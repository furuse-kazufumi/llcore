...
> 最終更新: 2026-06-17。現作業の正本は **memory harness 1 本 + Kaggle 導線 4 本 + 対応テスト 5 本 + docs**。現物 `git status --short` はこの更新時点で **今回の 3 スクリプト / 3 テスト / docs のみ dirty**。Kaggle focused gate の最新は **4 ファイル集合で `50 passed`**、memory harness を含む focused gate の最新は **`57 passed`**、今回の局所 gate (`test_memory_footprint_harness.py` / `test_build_kaggle_lm_compare_bundle.py` / `test_kaggle_push_readiness.py`) は **`31 passed`**、対応 `mypy` / `ruff` も通過。現行 `kaggle_push_readiness.py` は push credential (`kaggle.json` または `KAGGLE_USERNAME`+`KAGGLE_KEY`) の存在 + `kaggle kernels list -m --page-size 1 --csv` 疎通で auth を見ており、CPU bundle は quota check を skip、GPU bundle のみ `kaggle quota -v` を使う。live 実測は **CPU bundle の ready path (`rc=0`)** まで確認済みで、GPU bundle も試したが **ローカル Kaggle CLI 2.2.1 の `quota` サブコマンド自体が `not enough values to unpack (expected 2, got 1)` で失敗**したため quota live path は確認不能。実 `kaggle kernels push` は未実施。なお `kaggle quota -v` は週次の GPU 合計時間しか返さず、readiness green でも **`machine_shape` 別の空き枠**までは保証しない。下の旧 `p1_compare` / staged 2 件前提は**履歴メモ**であり、再開判断には使わない。
