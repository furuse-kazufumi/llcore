...
- Throughput is secondary evidence only in these tracked pilots. The historical `pilot*` artifacts were captured with `throughput_repeats=1`, so long-prompt marginal decode estimates can be noisy; when rerun at `throughput_repeats=3`, the previously suspicious `pilot240_seed2026` recurrent `prompt_len=256` point collapsed from a pathological estimate to a sane but low `decode_tok_per_s≈8.75`.
