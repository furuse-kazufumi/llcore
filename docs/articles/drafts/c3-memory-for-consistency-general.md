...
東大+香港科技大広州の研究 **MangaFlow**(arXiv:2605.28173)は、これを「**記憶**」で解決しました。各場面に「このキャラはこういう見た目」という参照を**記憶として持っておき、次のコマで引いてくる**。実験で「記憶を外すと一貫性が落ちる」ことも確かめています(一貫性のスコアが約 0.67→0.55 に下がった、研究者の自己申告)。
...
2. **「配置の正確さ 100%」は土俵が違う**。座標で明示的に置く設計なら、ほぼ当たり前に出る自作の指標で、ゼロから絵を生成する手法と同じ土俵では比べられません。しかも自分で設計したテストなので、有利な数字が出やすい点にも注意が必要です。
