# open weights のライセンスが 3 種に割れた — 「ローカルで商用に使えるか」の実務地図
...
2026 年前半の「3 種に割れた」状況は、**「open」の一語が商用可否を保証しない**ことを改めて示しました。Gemma 4 の Apache 2.0 化は本物の追い風。一方で OpenMDW のような「open だが OSS ではない」ライセンスは、条文を読むまで判断を保留すべき。**重みが落とせること(技術)と、商用に使えること(ライセンス)を、別々に検収する** —— これが、honest disclosure をライセンス面に当てた作法です。
