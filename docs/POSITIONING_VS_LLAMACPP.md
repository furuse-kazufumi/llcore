...
> 既存資産**(llcore は圧縮では再導出=競合しない)。llcore の列で他社と質的に異なるのは「**accuracy ベース
> 合否ゲートの有無**」だけ — そこにだけ llcore の固有性がある。**ただし char-LM スケールでの話**であり、
> 8B 級実モデルで同じ 97% ゲートを通せるかは **未実証**(llcore 自身の留保)。規模差(約 15,000×)を捨象しない。
