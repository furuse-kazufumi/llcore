...
<img src="assets/deceptive_corridor.svg" alt="だまし地形と進化" width="660">

> ふつうの山登り（赤）はニセの頂上で止まり、進化＝いいものを選んで残す（緑の群れ）は谷をとび石で渡って本物の頂上へ。**手順4 はこれを人工の「だまし地形」で実証した（③が効く状態の存在証明）。** Step C はこれが実際の記憶問題でも自然に起きるかを試した。

## 2. 背景 — ③ がここまで「空転」してきた経緯と本 Step C の位置

本 verdict が問うのは **「実タスク本来の性質から欺瞞 corridor が自然発生し、そこで ③ が load-bearing になるか」** の一点。上流の確定事実を逐語参照しつつ、本 Step C はその CPU 手順の延長として実記憶タスクで決着を試みる。結果は Yes/No の two-way ではなく **N/A (本基質では③を有効分離できる欺瞞回廊が現れず、clean な③検定が未達)** の決着である (§4)。
...
<img src="assets/step_c_two_regimes.svg" alt="Step C の2つの結果" width="660">

> 実際の Step C では「だましだが渡れる坂道」が現れず、片方は**湖の底**（delayed_parity=全員ゼロ＝脳が解けない）、もう片方は**天井**（flip_flop=全員ほぼ満点＝かんたんすぎ）。どちらも③の差が見えない（床と天井で別々に潰れた）。
