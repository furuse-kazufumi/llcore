...
**複数 reservoir 結合 (深さ/並列乗法/2次readout/進化/超ワイド/全部入り) では、本設定下で
5-bit parity の床は外れない。** 深さは表現力を統計的に有意に上げる (2L/3L strict PASS) が
絶対値 0.05–0.10 に留まり degree-5 の壁を越えない。degree-2 readout の不能のみ positive control
で原理的に確定済 (reservoir+線形側は本設定で極めて頑健な床であり、パラダイム全体の不可能性
証明ではない)。
