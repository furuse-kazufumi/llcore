...
AI に「いい設計」を見つけさせる作業は、**霧の中で一番高い山をさがす登山**にそっくりです。場所の「高さ」を設計の「よさ」だとすると、一番高い頂上が「一番いい設計」。この文書は、その山さがしを、みんなが知っているもの — **山登り・川・湖・太陽・海**、それに「ものを覚えるコツ（記憶術）」 — にたとえて、実際の実験とつなげたメモです。

**いちばん知りたいこと:** ただ「今より高い方へ歩く」だけの登山だと、**ニセの頂上で止まってしまう「いじわるな地形」**があります。そこで「いろんな登り方をぜんぶ残しておいて、谷をとび石で渡る」やり方が役に立つはず。これは生き物の進化の「**いいものだけを選んで残す（③）**」と同じ考え方です。これが本当に「ふつうの登山より高く登れる」のかを、お金のかかる大きなコンピュータ（GPU）ではなく、**安いコンピュータ（CPU）で、効く・効かないの両方をはっきりさせる**のが Step C です。

**読むときの注意:** たとえ（山・川・海）は**頭の中で考えるための道具**で、そのまま本当のことではありません。この文書の「〜のはず」は、ぜんぶ実験で確かめてから採用します（うのみにしない）。もっとやさしい説明は [`YOUGO_平易版.md`](./YOUGO_平易版.md) に。
...
### 8a. 2つのパラダイム
- **地形 (landscape) パラダイム** (現 Step C): Wright 1932 の適応度地形。**外部から固定された目的(地形)**を 山登り (hill-climb)/航行する。適応度 (fitness) は実験者が与える静的写像。
  → ③(選択圧)も④(過剰繁殖)も**外から課す**構造 = §7a の「④空転」と §6b の 交絡 (confound) は**この枠組みに内在**する。
