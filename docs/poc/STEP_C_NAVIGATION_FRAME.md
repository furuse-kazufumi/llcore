...
AI に良い設計を見つけさせる作業は、**霧の中で一番高い山を探す登山**に似ています。設計の良さを「土地の高さ」で表し（高い＝良い設計）、一番高い頂上＝最良の設計を探します。本書は、対話で生まれた「**どうやって頂上を探すか**」のたとえ — 記憶術（記憶の宮殿）／川の流れ／湖に浮かぶ小島／空を動く太陽／そして海そのもの — を、実際の実験の仕組みと「確かめられる予想」に結びつけた覚え書きです。

**核心の問い:** ただの「山登り」（今より少し高い方へ進むだけ）だと、**ニセ頂上で止まる「だまし地形」**があります。そこで「いろんなタイプの設計を捨てずに残しつつ、飛び石で谷を渡る」やり方（**品質多様性, QD**）が、進化の本質「**選んで残す（③適者生存）**」として本当に効くのか — それを高価な GPU でなく **CPU で安く、効く／効かないの両方向に決着**させるのが Step C です。

**読むときの注意:** たとえ（山・川・海）は**考えるための道具**であって物理的事実ではありません。本書の予想はすべて実験で確かめてから採用します（鵜呑み禁止）。詳しい平易版は [`YOUGO_平易版.md`](./YOUGO_平易版.md)。
