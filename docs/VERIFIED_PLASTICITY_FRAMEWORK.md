...
近年、自己改善・継続学習を謳う著名 OSS が多数ある(2026-06-10 競合スキャン):hermes-agent(NousResearch, 189k★)は「20+ スキルで 40% 高速」、ECC(211.8k★)は Continuous Learning、headroom は learn を掲げる(各社リポジトリ記載・2026-06-10 時点・第三者未検証)。**これらの性能/安定性の優位主張は、いずれも第三者未検証の自社ベンチである**(star 数は人気の証であって性能優位の証ではない)。本記述は競合を貶めるものではなく、「**未検証である**」という事実のみを述べる。
...
世界モデル系の手法は、一般に安全設計に寄与しうるが、**形式的な保証(guarantee)を与えるものではない**。これは技術コミュニティで広く共有される観察であり、2026 年の講演でも同趣旨が示された(藤吉弘亘氏)。verified-plasticity は、対照的に sound certificate によって **GUARANTEE(証明付き安定)を出す**点で異なる(ただし small-n per-component 域に限る、という honest 限定付きで)。

機械学習は、特徴量や構造を人が設計する段階から、モデル自身がそれらを学習で獲得する段階へと進んできた(一般的な技術的事実)。本研究の進化テーゼ(構造を固定せず online に獲得する)はこの流れと同型であり、本枠組みは、そうした自己獲得的な構造適応に対し、**発散しないことの保証**を後付けで課す装置と位置づけられる。
