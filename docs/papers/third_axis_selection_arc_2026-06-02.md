...
The structural insight that organizes these results: behavioral niching is load-bearing only when the deception lives in a **high-dimensional behavior space** that direct sampling cannot reach. In our synthetic corridor the behavior descriptor is the mean of a 24-dimensional genotype; by central-limit concentration the global peak is a measure-zero region that random restarts cannot hit, so the archive's stepping-stones become essential. The realistic substrates fail this condition: their hard coordinates (e.g. leak rate, discrete kernel choice) are one- or low-dimensional, and a hill-climbing baseline with restarts solves them directly. We note an illustrative biological precedent — Wright's shifting balance theory and the Coyne–Barton–Turelli verdict that diversity-maintaining selection is rarely decisive because real fitness landscapes are usually simple — as structural grounding (an analogy, not proof) for both the condition and our negative result. We discuss why this leaves high-dimensional full-LLM loss landscapes as the only remaining venue for the third factor, and why even there a strong gradient baseline poses the same risk.
...
### 2.5 Biological grounding: Wright's shifting balance

The boundary condition we are after — diversity-maintaining selection earns its keep only in a narrow regime and is redundant outside it — has an unusually clean precedent in 20th-century evolutionary biology. We draw it as an **analogy, not an attribution**: the correspondence is structural, and several junctures of the mapping are loose. We flag those caveats explicitly here and in §6.

**Shifting balance as the precedent for ③.** Sewall Wright's shifting balance theory (foundations in Wright 1931, *Genetics* 16:97–159, and Wright 1932, *Proc. 6th Int. Congr. Genetics* 1:356–366; the named, numbered three-phase articulation was a later codification) holds that a single large *panmictic* population under deterministic mass selection is trapped on its local fitness peak, because crossing a valley requires transiently *lowering* mean fitness — which deterministic selection refuses. Wright's remedy is to subdivide the population into many partially isolated demes: **Phase I**, random genetic drift carries some small demes down across a valley; **Phase II**, ordinary within-deme mass selection drives them up a new, higher peak; **Phase III**, interdeme selection spreads the superior gene combinations species-wide. The metapopulation collectively crosses valleys a single converging population cannot — the biological statement of "stepping-stones across a deceptive valley."

The mapping to ③ / MAP-Elites is direct but informal: each archive cell behaves like a quasi-isolated deme, local elitism within a cell like within-deme mass selection, and cross-cell variation like interdeme spread, with the *archive collectively* (≈ the metapopulation, not any single cell) crossing low-performance valleys. Two honesty points bound the analogy. First, it is a commentators' framing: the foundational MAP-Elites paper (Mouret & Clune 2015, arXiv:1504.04909) and the QD literature **do not cite Wright or "shifting balance"** — we invoke Wright as inspiration, never as MAP-Elites' lineage. Second, the mechanism is structural, not identical: MAP-Elites crosses valleys via *variation operators* that place offspring in new cells, **not by genetic drift**, and an archive is not a population of replicating cells.

**The Wright–Fisher axis is the dimensionality axis.** R. A. Fisher (*The Genetical Theory of Natural Selection*, 1930) held that large, effectively panmictic populations under mass selection on additive variance were the dominant engine of adaptation; he conceded drift occurred but judged it quantitatively negligible in realistically large populations (he did not deny it). The deepest axis of the disagreement was **epistasis / gene interaction and the *shape* of the fitness landscape**: Wright assumed a *rugged, multi-peaked* landscape from non-additive interactions (hence the need for drift to cross valleys); Fisher granted such interactions exist but judged them unimportant, implicitly favoring a single-peak, smoothly-ascending landscape where mass selection suffices. This epistasis/ruggedness axis is exactly the dimension our finding lives on. **Landscape topology is the whole question** — when the landscape is genuinely rugged and high-dimensional (Wright's regime), diversity-maintenance crosses valleys; when it is smooth or its hard coordinate is low-dimensional (Fisher's regime), mass selection — the biological analogue of a strong random-restart hill-climber — already suffices.

**Two real-world poles.** Evolutionary biology supplies a clean exemplar for each end of the dimensionality axis.

- *Low-dimensional pole — peppered-moth industrial melanism (the analogue of our BG9 kernel case).* The *carbonaria* (black) vs *typica* (pale) polymorphism in *Biston betularia* is controlled by a **single Mendelian locus with a small, discrete allele set** (the causal mutation is a transposable-element insertion in the *cortex* gene; van't Hof et al. 2011, 2016; Cook & Saccheri 2013), under **strong directional selection** (*s* ≈ 0.1–0.2; Saccheri et al. 2008; predation re-confirmed by Cook, Grant, Saccheri & Mallet 2012). The optimum is unimodal at any given time and shifts with the environment; **simple directional selection — the biological analogue of greedy hill-climbing / random-restart — directly fixes the fitter morph, and no diversity-maintaining mechanism is needed or invoked.** This is precisely the BG9 kernel-union situation: kernel choice is a single low-dimensional discrete coordinate (four options) that a random-restart hill-climber samples directly, so ③ cannot structurally separate from it.
- *High-dimensional contingent pole — Lenski's Cit+ innovation (the ③ regime's antecedent).* Aerobic citrate utilization evolved in **exactly 1 of 12** *E. coli* populations of the Long-Term Evolution Experiment, first detected near generation 31,500 (Blount, Borland & Lenski 2008), via an *ordered* accumulation of potentiating mutations followed by a tandem-duplication actualization event (Blount et al. 2012). Replay experiments distinguished **historical contingency / potentiation** from a constant-rate rare mutation. This case genuinely exemplifies contingency, epistasis, and the value of exploring a high-dimensional rugged landscape — a fair real-world picture of the regime in which ③ *can* matter. But it maps only to the **antecedent** of our conditional, not its headline (see §6): the LTEE used no niching *algorithm*, and its 12 parallel populations are themselves a random-restart-like design, so it is an existence proof for the high-dimensional regime, not evidence that niching beats a strong restart baseline.
...
**A computational echo of a biological debate.** Our negative result has a striking biological mirror in the empirical verdict on Wright's shifting balance (§2.5). Coyne, Barton & Turelli (1997, *Evolution* 51:643–671; reaffirmed in their 2000 reply) evaluated the three-phase theory and concluded that *mass selection usually suffices* — there are "few empirical observations explained better by Wright's three-phase mechanism than by simple mass selection" — and that shifting balance is decisive only under restrictive, rarely-met conditions, since population-structure estimates imply random drift can cross only *shallow* valleys while most adaptations require no valley-crossing at all. Translated into our terms: when the landscape is not genuinely deceptive/high-dimensional, plain mass selection (≈ a strong random-restart baseline) already solves the problem, and the diversity-maintaining apparatus buys little. Their quantitative point — real landscapes are usually simple, so the diversity machinery is rarely the deciding factor — is the biological statement of exactly our finding. We stress this is *analogy, not proof*: it explains why a negative result of this shape is unsurprising, but it does not establish our computational result, which stands on the experiments of §4. Three honesty bounds apply (elaborated in §6): Coyne–Barton–Turelli did not "disprove" shifting balance, the debate remains live rather than settled, and one of its mechanisms (Phase III gene-flow trapping) is a *stronger* claim than ours with no computational counterpart in our stateless setting.
...
### 6.1 Limits of the biological grounding

The Wright / Coyne–Barton–Turelli material in §2.5 and §5 is **illustrative grounding, not evidence**. The biology explains *why* a negative result of this shape is unsurprising and *when* ③ should be expected to add nothing; it does not demonstrate our computational result, which rests on §4 alone. Four specific looseness points must travel with the analogy and should not be over-read:

- **The analogy is structural, not mechanistic.** MAP-Elites crosses valleys via variation operators, not genetic drift; an archive is not a population of replicating cells; and MAP-Elites is not cited by, nor cites, Wright. The correspondence is "diversity + parallel local hill-climbing + cross-niche transfer," not the same physical process.
- **Biology's Phase III is a *stronger* claim than ours, with no computational counterpart.** In shifting balance, the gene-flow barriers that preserve diversity can actively *trap* good solutions in marginal demes and block their species-wide spread — so niching there can be *counterproductive*, not merely unnecessary. Our stateless, discrete-choice setting has no analogue of this gene-flow / recombination cost; we do **not** claim it, and the analogy is loosest precisely here.
- **The Lenski Cit+ case maps only to the *antecedent* of our conditional.** It is an existence proof that contingency + standing diversity can enable rare innovations in a high-dimensional regime — *not* a controlled demonstration that a niching algorithm beats random restart. The LTEE used no niching operator, and its 12 parallel populations are themselves a random-restart-like design (Cit+ still required ~31,500 generations in one of twelve). It supports the regime in which ③ *can* help; it is not evidence that ③ outperforms a strong restart baseline. (We also note the contested rapid-direct-selection follow-up of Van Hofwegen, Hovde & Minnich 2016 [citation to confirm], which disputes the "extraordinarily rare" framing.)
- **The peppered-moth heterozygote-advantage question is unresolved.** While the dominant and sufficient explanation for industrial melanism is low-dimensional directional predation-driven selection (matching our BG9 kernel case), there is a genuine, peer-reviewed and still-open debate over whether the ~1970 North Wales cline required a non-visual heterozygote-advantage (a balancing/diversity component) in addition to predation. We rely on the *dominant* explanation, not a claim that no non-directional mechanism is involved.
...
The biological-grounding material (§2.5, §5, §6.1) is drawn from the verified companion artifact `docs/papers/biological_grounding_third_axis.md`, where each reference's existence, attribution, and load-bearing content were adversarially verified. References used in-text with full-text/DOI verification: Wright (1931, 1932); Fisher (1930); Coyne, Barton & Turelli (1997, 2000); the peppered-moth set (van't Hof et al. 2011, 2016; Cook & Saccheri 2013; Saccheri et al. 2008; Cook, Grant, Saccheri & Mallet 2012); and the Lenski Cit+ set (Blount, Borland & Lenski 2008; Blount et al. 2012). The single in-text reference still requiring manual confirmation before submission is flagged inline as "[citation to confirm]": Van Hofwegen, Hovde & Minnich (2016). Other companion-artifact references that were *not* promoted into this paper's body (Goodnight & Wade 2000; Wade & Goodnight 1998; Peck, Ellner & Gould 1998; Stanley, Lehman & Soros 2017; Nordmoen et al. 2021) carry the same "needs manual confirmation" flag in the companion and are deliberately omitted here. The biology is illustrative grounding for the computational finding, not proof of it.
