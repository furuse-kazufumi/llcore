...
**Neuroevolution.** The outer-loop motivation — evolving the dynamics of neural systems — sits in the neuroevolution tradition; our search itself is a minimal genetic algorithm (§3.5) rather than a Quality-Diversity method, so we note MAP-Elites and the broader Quality-Diversity programme (Mouret & Clune [verify]) only as adjacent context for the outer loop, not as a method we use here.
