...
- **A near-completeness result on CPU.** With an accurate solver, the quadratic SDP is nearly complete: degree-4/6/8 SOS rungs add only a handful of near-boundary genes, six of the residual ten are *correctly-rejected* switched-expansive genes (not misses), and ~2 near-boundary genes remain open at finite CPU lift degree — an honest asymptote toward the JSR = 1 boundary rather than a closed frontier.
- **The methodology core: catching our own solver artifact.** We document, reproducibly, how cvxpy's default first-order solver (SCS) false-negatives near the SDP feasibility boundary and fabricated a non-nested "complementary" degree ladder and an inflated residual. Pinning CLARABEL collapsed the artifact (64% → 95% quadratic-SDP coverage; "+54" extra coverage → "+4"; a 23/13/18 split → a nested 0/0/54). The transferable lesson is that margin-sweep red-teams are *structurally blind* to this failure, and the decisive checks are solver-swap and multi-perspective adversarial review.
...
- **degree-4 SOS adds 0 on top of the SDP**, and degree-4 is *nested* in degree-6 (degree-4-minus-degree-6 = 0), so degree-4 contributes no unique coverage;
- **degree-6 SOS adds 1** uniquely (cumulative 287; counting degree-4-and-degree-6 together adds +3 over the SDP, for a degree-≤6 residual of 10);
- of the **10** the degree-≤6 ladder leaves: **degree-8 SOS closes 4**; an **exact-JSR bracket closes 2 of those 4** finite-gap genes; **2 near-boundary genes** (jsr_lb 0.9915 and 0.9787) stay open at finite CPU lift degree; and **6 of the 10 are correctly-rejected switched-expansive genes** — pointwise-ρ < 1 at each vertex but a vertex *product* expands (JSR ≥ 1), so the gate rightly refuses them. They are not misses.
...
1. **The certifier theorems.** For the quadratic certificate, a feasible common vertex-LMI implies stability of the convex hull by convexity of the quadratic form, which implies the nonlinear map contracts. For degree ≥ 2, the lifted full-space SOS certificate is a *sufficient* condition for contraction. These are the guarantee; they are mathematical, not solver-dependent.
...
3. **A one-sided JSR falsifier.** The JSR product oracle (a Gripenberg lower bound over products up to length 6) is a *falsification* check: a certified gene with jsr_lb ≥ 1 would *prove* an unsound admit. We observe **0** such genes across every corpus (worst certified jsr_lb 0.966 on the relevant battery). This is *necessary but not sufficient* evidence — "**0 observed false admits**," not an end-to-end machine-checked proof at float precision.
...
**SOS and Lyapunov certificates.** The sum-of-squares relaxation of Lyapunov stability, and its formulation as a semidefinite program, originates with Parrilo's thesis and subsequent work [verify]. The specific lifting of switched/uncertain linear dynamics through a polynomial (Veronese) map to obtain SOS Lyapunov certificates, and its connection to the joint spectral radius, is due to Parrilo & Jadbabaie [verify]. Our degree-4/6/8 certifiers and the non-monotonicity we observe sit squarely in this literature.

**Joint spectral radius.** The JSR and its approximation are studied by Parrilo & Jadbabaie [verify], by Gripenberg's branch-and-bound lower/upper bracketing [verify], and comprehensively by Jungers [verify]; the NP-hardness and undecidability results that motivate our "asymptotes rather than closes" framing are part of this body of work [verify]. We use a Gripenberg-style lower bound as our one-sided soundness falsifier.
...
5. J.-C. Liu and M. Xiao, or the relevant JSR NP-hardness/undecidability results (Blondel & Tsitsiklis, "The boundedness of all products of a pair of matrices is undecidable"). *Systems & Control Letters*, 2000. [verify]
...
18. W. Lyapunov. "The general problem of the stability of motion" (translated, *International Journal of Control*, 1992). [verify]
