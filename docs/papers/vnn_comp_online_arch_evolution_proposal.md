...
10. **Spec ↔ reference-implementation conformance gap.** The benchmark spec (§4.1) prescribes real `.onnx` and `.vnnlib` inputs, but the reference implementation (`scripts/poc_7a_vnn_comp_reference_impl.py`) does **not** parse them: `parse_model_onnx` and `parse_invariant_vnnlib` raise `NotImplementedError` for `.onnx`/`.vnnlib` files and accept only a JSON dummy (`{"blocks": [...]}` for the network, `{"state_bound": ..., "max_input_abs": ...}` for the invariant). The reference impl is therefore a *minimal skeleton*: it fully supports a single `RwkvTimeMix` kernel and a single `ChangeOp` family (`reparam_inplace`), partially supports `insert_subblock`, and returns `unsupported` for `delete_subblock`/`reorder_subblocks`/`type_subst`. It is an existence proof that the §4.5 protocol is implementable, not a spec-conformant verifier. Closing this gap — a real VNN-COMP ONNX-subset parser and `.vnnlib` S-expression parser — is required before the reference impl can ingest community-supplied benchmarks, and is enumerated as a near-term task in §10.
