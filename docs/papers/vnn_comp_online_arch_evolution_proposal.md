...
### 2.9.1 Incremental verification across *changing networks* (the closest prior art)

Three recent lines target the precise relation our category generalises — proving a property survives a *change to the network itself*, by reusing rather than discarding prior proof effort — and none has a standardised competition slot. **IVAN** (Ugare et al., PLDI 2023; arXiv:2304.01874) is the canonical "different-network, same-property" work: given an *updated* DNN (e.g., after fine-tuning or quantisation) it reuses the parent network's verification results to re-verify the same robustness property at reported geometric-mean speedups of roughly 2.4×–3.8× over verifying from scratch (varying by benchmark family). This is exactly the proof-reuse-across-related-networks primitive that `online-arch-evo` generalises — except IVAN incrementalises across *one* network update, whereas our category demands *per-step bounded* re-verification across an unbounded `ChangeOp` stream, with a refusal/rollback option and a wall-clock budget per step. **MUC-G4** (Li & Li, arXiv:2506.04268, 2025) sits in the same niche from the compression angle: it verifies a *structurally changed* network (quantisation/pruning) against the same property by classifying the change and reusing Minimal Unsat Cores from the original proof. Its change set is a fixed compression operation rather than our open-ended `ChangeOp` taxonomy, which makes it a sharp contrast for arguing that the *unbounded, online* framing is what is new.

The demand for this capability is independently visible from the *learning* side. **Certified Continual Learning for Neural Network Regression** (Pham & Sun, arXiv:2407.06697, 2024) preserves a network's already-certified correctness properties as the network is *retrained* under continual learning — the "same-invariant must survive a changing network" problem, framed as a learning method rather than a verification benchmark. Together these works establish that property-preservation under network change is an active research demand, while none of them offers a standardised *incremental-verification benchmark category* — which is the gap `online-arch-evo` is written for. We also note adjacent *online formal-verification* threads that change a different object: adaptive runtime shielding either repairs the *specification* online to keep it satisfiable as environment assumptions drift (Georgescu et al., arXiv:2511.02605, 2025) or infers hidden environment parameters to filter unsafe actions with bounded runtime overhead (Kwon et al., arXiv:2506.11033, 2025) — in both cases holding the controller *architecture* fixed. These share our "maintain an invariant as something changes, with bounded online updates" DNA but stream *spec/assumption* changes over a *fixed* network, whereas we hold the invariant *fixed* and stream *network-structure* mutations — the orthogonal axis, and a useful contrast for positioning the category.
...
10. **Reference-implementation scope (partial spec conformance).** The reference implementation (`scripts/poc_7a_vnn_comp_reference_impl.py`) now parses real `.onnx` and `.vnnlib` inputs (this was previously a gap). `parse_model_onnx` reads a RwkvTimeMix block chain encoded as custom-op nodes (op_type `RwkvTimeMix`, domain `llcore.rwkv`, float attributes `decay`/`mix`/`gate_str`; requires the optional `onnx` package), and `parse_invariant_vnnlib` reads a scalar `X_0`/`Y_0` box-bound `.vnnlib` subset via a stdlib S-expression parser; a companion seed-based `generate_properties.py` emits instances that round-trip through these parsers and feed the verifier (a JSON-dummy path is retained for dependency-free tests). What remains a *skeleton* is the model class rather than the I/O: a single kernel (`RwkvTimeMix`) and a single fully-supported `ChangeOp` family (`reparam_inplace`), scalar state, and a custom-op ONNX encoding that is a *documented convention* (RwkvTimeMix is not a standard ONNX operator). Broadening the kernel/operator vocabulary and supporting multi-dimensional state remain near-term tasks (§10); the reference impl is thus a credible existence proof of the §4.5 protocol, not yet a strong baseline.

11. **`sat`-verdict witnesses are not yet implemented, so no audited TPR/FPR comparison is possible.** The reference impl emits independently-auditable soundness witnesses only for `unsat` verdicts: `write_unsat_witness` produces a `.smt2` file that an external checker (`z3 witness.smt2`) can re-verify, but for a `sat` verdict the impl explicitly records only a textual detail and returns no witness path ("sat witness emission not implemented in PoC 7a"). The spec (§3.1, §5.1) requires `sat` to exhibit a concrete counter-example; until that witness is emitted and made externally checkable, the false-positive side of a verdict stream cannot be audited end-to-end. Consequently the audited true-positive / false-positive-rate comparison against α,β-CROWN that §9 item 6 calls for cannot yet be carried out: only `unsat` claims (the true-negative/soundness side) currently have mechanically auditable evidence.
