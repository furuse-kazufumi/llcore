...
2. **メモリ効率の実機計測** — #33/35/38/39/40/56/59, bank:A2/A3/A4/A5/A6, comp:8/9/10, draft:a7。mmap 固定コスト・working-set 上限で RAM超・streaming の allocator 壁・文脈長×8 で GPT peak×2.65 vs recurrent×1.00。pivot(#35)後の勝ち筋実証。**#59=支配項は言語ランタイム baseline(torch +184MB=モデルの142×)→ Rust/native の効きどころ**。
...
| 59 | int8で4MB削った隣で言語ランタイムが184MB食っていた(Rust論点の実測回答) | S | 技術設計/honest/業界比較/教訓/戦略 | 生seed | memory-efficiency連載・技術者向け中核 |
...
- **#9 / #11 / #12 / #13 / #15**: (caveat なし)
- **#57**: 「捏造」は AI 批評家(gem-critic)の誤審=false-positive。私の 49.23→13.97MB は `int8_footprint_bytes` の resident-weight 会計(params+buffers)の実出力で正しく、批評家の 47.66MB は params-only 会計=差 1.57MB は causal-mask buffer(6層×256×256×4B)。**会計基準(params/buffers/RSS/on-disk)を明示しないと同じモデルでも別人の数字になる**。教訓2=AI の所見も一次証拠で一件ずつ検証(`feedback_no_solo_ai_judgment`)— 検証しなければ正しい数字を捏造扱いで書き換えていた(冤罪執行)。挿絵=スナックバス江「極刑でよくない？／よくないのよ」(内容で同定=`bazue_all/162.jpg`。番号162で引いたら catalog では別コマ=本記事の教訓の再演)。公開はユーザー指示(2026-06-19)で `bazue_all` の生crop を直接使用可(ライセンス形態の最終判断はユーザー。`reference_alu_manga_crops` の Alu-permalink-only ルールは本件緩和)。
- **#58**: 判定を純関数 `_should_promote` に切り出し決定的にテスト。ランダム未学習モデルの top-1 は near-uniform で int8 ノイズに脆く retention が flaky=「テストしにくい=設計が不明瞭」という TDD のシグナル通り(superpowers TDD)。
- **#59**: 実測 RSS=baseline 13.4MB→+torch 197.3MB(tax +183.9MB)→+model 213.6MB、int8 1.51MB=142×。量子化/mmap/定数状態は言語非依存で支配項は torch baseline=Rust/native(llama.cpp/candle)の効きどころ(~180MB 削減 > int8 数MB)。honest 反対側=Rust は int8/mmap/定数状態を改善しない・素の matmul は BLAS より遅い・184MB tax が効くのは豆モデル規模ゆえ(GB 級では baseline 無視可)=Rust baseline 優位は小モデル小RAMで最大。candle 実測は未取得=主張の最終確証は要計測。
