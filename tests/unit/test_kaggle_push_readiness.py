...
def test_verify_push_payload_snapshot_rejects_missing_dataset_block_for_dataset_mode(tmp_path: Path) -> None:
    script = _load_script("kaggle_push_readiness.py")
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    (bundle_dir / ".kaggleignore").write_text("", encoding="utf-8")
    ...
    with pytest.raises(ValueError, match="missing dataset block for dataset-mode bundle"):
        script._verify_push_payload_snapshot(bundle_dir, snapshot)


def test_verify_push_payload_snapshot_rejects_dataset_claim_for_embedded_bundle(tmp_path: Path) -> None:
    script = _load_script("kaggle_push_readiness.py")
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    (bundle_dir / ".kaggleignore").write_text("", encoding="utf-8")
    (bundle_dir / "runner.py").write_text("# ok\n", encoding="utf-8")
    (bundle_dir / "bundle_manifest.json").write_text(
        json.dumps({"kernel_id": "furusekazufumi/expected"}),
        encoding="utf-8",
    )
    snapshot = tmp_path / "snapshot.json"
    snapshot.write_text(
    ...
    with pytest.raises(ValueError, match="claims dataset_source for embedded bundle"):
        script._verify_push_payload_snapshot(bundle_dir, snapshot)
