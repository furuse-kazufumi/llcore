...
- **検出力の自白 (H2)**: 16 seeds で CE の小効果は拾えない可能性 — その場合「検出せず」を報告 (盛らない)。
- **検出力の自白 (H1; 敵対レビュー指摘で追加)**: 三重保守則 (max-p 連言 × 方向一致 × Holm) の
  検出力を MC で見積もった (feasibility n=32 の seed 分散ベース, Poisson + binomial thinning,
  4000 iter):
  - **均一効果なら問題なし**: base 窓死亡率 6% (F 条項ぎりぎり) × 相対 30% 削減でも
    per-n power ≈ 0.99 (3n 同時 ≈ 0.98)。
  - **効果が seed 間で異質なら急落**: benefit 75% (−40%) / harm 25% (+20%) の混合で
    3n 同時 0.46、60/40 混合で 0.05。feasibility の honest prior (§0-8: OBSERVE は NONE より
    死を減らせていない) を踏まえると異質・逆方向の可能性は現実的。
  - 帰結: **H1 不成立は「OBSERVE/empirical 無効」の証拠ではない** (検出力 × 異質性の交絡)。
    この caveat は §5 解釈マップに従い結果報告に必ず併記する。
