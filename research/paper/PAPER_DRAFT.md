...
- **A sound trajectory-tube gate occupies the verified-memory-evolution corner at PoC scale (§9).** Upgrading the single-step `L < 1` certificate to a closed-form disturbance-tube bound `limsup‖e_t‖∞ ≤ G·w̄/(1−L)` and gating evolution on `r ≤ r_max` yields a strict subset of the contraction gate (640 extra rejections; constructive witness), passes an empirical 0/180 tube cross-check, and — under a pre-registered confirmatory test (n = 20 seeds, sign-flip permutation) — measurably *helps* fitness on the longest memory horizon (delay 8: mean Δ = +0.0152, p = 0.0056) while showing no detectable effect on the memory-free control tested (exploratory): a dose–response in the memory horizon, consistent with a retention guarantee rather than a generic booster.
