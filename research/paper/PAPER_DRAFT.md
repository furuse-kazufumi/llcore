...
- **A sound trajectory-tube gate occupies the verified-memory-evolution corner at PoC scale (§9).** Upgrading the single-step `L < 1` certificate to a closed-form disturbance-tube bound `limsup‖e_t‖∞ ≤ G·w̄/(1−L)` and gating evolution on `r ≤ r_max` yields a strict subset of the contraction gate (640 extra rejections; constructive witness), passes an empirical 0/180 tube cross-check, and — under pre-registered confirmatory tests (sign-flip permutation) — measurably *helps* fitness on the longest memory horizon (delay 8: pooled n = 40 mean Δ = +0.0134, p = 0.0021; single-run significance seed-sensitive) while a *mechanically independent* disturbance-magnitude axis shows no effect (pre-registered negative, p = 0.59): a dose–response in the memory *horizon* (the retention-margin/denominator axis), not a generic invariant-load booster.

- **Who holds the verifier: memory formation under viability threat (§9.6).** Moving the sound gate *inside* the gene is worthless on a bounded substrate (pre-registered null: re-adaptation AUC Δ = −0.04, *p* = 0.67 — contraction decoupled from survival), but on substrates where an environment step in recurrence gain makes stable genes diverge, an endogenous sound gate eliminates steady-state lethal evaluations entirely (0.0 vs 27.2 ungated, *p* < 0.001; robust 12/12 across pre-registered seed/κ/dimension/task-difficulty configurations with a capability guard — worst gap −0.0072), certificate-preserving repair carries population memory *through* death (pop-mean Δ = +0.060, *p* = 0.0011), and a 2³ factorial shows one sound gate subsumes stacked empirical mechanisms (E×O interaction *p* = 0.0003 / 0.0045 by substrate) while stacked empirical mechanisms never reach it. Under premise-broken sensing, a trust meta-controller tracks the regime but pays off only where premise violations are costly — the pre-registered overall hypothesis fails on one substrate and is reported.

- **Grounding the hierarchy in gradient training (§9.7).** A pre-registered run on the §7 substrate (n = 64–256 × 16 seeds, T4) grounds the §9.6 hierarchy where drift is real: empirical avoidance with a *shared* death history reduces window death rates at every dimension (**H1 PASS**, Holm *p* = 0.027; counting-artifact discipline passed — ρ-excess 72.6 → 19.0 at n = 256) but never approaches the sound gate's zero deaths, while *self-history-only* avoidance is useless or harmful (worse than unsupervised at n = 128) — sound ≫ shared-empirical ≫ self-empirical ≈ none. The repair-beats-rollback CE hypothesis is directionally uniform but fails its pre-registered conjunction (Holm *p* = 0.058) and is reported as **not supported**.

- **Limitations, reproducibility, and roadmap (§10).** The CPU runs are deterministic; n = 8 is a hard limit of the vertex-enumerating certifier (memory wall near n ≈ 16, infeasible at n = 32). We provide an authoritative reproduction table and the roadmap (vertex-free certifier R-LLM-1, gradient-trained GPU stage, multimodal, cost-as-selection L4).

The remainder of the paper follows these contributions in order: the substrate and certifiers (§2), soundness rigor and limits (§3), the reservoir byte-LM (§4), the L3 frontier (§5), the gradient cross-check (§6), the GPU scale-up and real-Transformer test (§7), vertex-free certification (§8), the trajectory-tube memory gate and the viability-threat memory-formation PoC (§9), and limitations/roadmap (§10).
...
- **Reproduction scripts and primary result JSON.** The following table is the authoritative list of
  ...
  | Second axis (w̄ disturbance), pre-registered (§9.4) | `research/verified_memory_poc/run_wbar_decision.py` → `results_wbar_decision.json` | **negative**: no dose–response on the disturbance axis (confirmatory w̄=0.20 p=0.59, non-monotone); gate binds throughout; pre-registered before results | verified_memory_poc/VERDICT.md §11 |
  | d8 pooled replication (§9.4) | `research/verified_memory_poc/run_d8_pooled.py` → `results_d8_pooled.json` | two independent n=20 runs pooled → n=40: Δ = +0.0134, p = 0.0021 (29/40 positive); single-run replication p=0.093 (seed-sensitive) | verified_memory_poc/VERDICT.md §11 |
  ...
  | R-endo bounded-substrate A/B (§9.6) | `research/internalization_poc/run_d_internal_ab.py` → `results_d_internal_ab.json` | self-verdict ≡ external gate 0/5000; H1 non-inferior; autonomy null (AUC Δ = −0.04, p = 0.67) | internalization_poc/VERDICT.md |
  ...
  | HD-1 grounding preregistration (§9.7) | `research/internalization_poc/HD1_GROUNDING_PREREG.md` + `hd1_grounding_kernel.py` + `analyze_hd1_grounding.py` (all committed before results) | binding two-stage registration; 4-lens adversarial review fixed 4 findings pre-commit (decidable equivalence δ=1/80; power confession; counting-artifact discipline; fail-fast corpus) | HD1_PREREG_REVIEW_2026_06_07.md |
  | HD-1 grounding main run (§9.7) | `hd1_grounding_kernel.py` → `result_hd1g_n{64,128,256}.json` (Kaggle T4, 256 records) | **H1 PASS** (Holm p=0.027; OBS_P2 < NONE all n; artifact discipline passed, ρ-excess 72.6→19.0 at n=256); **H2 not supported** (Holm p=0.058, direction uniform); pass-1 self-history worse than NONE at n=128; NONE ρ_hp=1.969 reproduces §7 band | HD1_GROUNDING_VERDICT.md + analysis_hd1g_output.txt |
...
follow-up implementation of SSGM** as of the lookup date, so the verified-memory-evolution corner
(§9) remains, to our knowledge, unoccupied by an implementation. A third sweep over the memory-core
...
> | A trajectory-tube gate strictly tightens contraction gating (640 extra rejections; 0/180 empirical tube violations) and, under a pre-registered confirmatory test, improves fitness on the longest memory horizon (d8: Δ = +0.0152, p = 0.0056, n = 20, 16/20 positive) | Holds (§9) | Scalar memory-update kernel + probe-based copy-task fitness; effect small and right-skewed; d0/d4 exploratory (no claim); the tube bound is parameter-derived (no trajectory-level solver query — the 64-seed run is a cross-check of the theorem, not part of gate soundness); coupled kernels face the box-mismatch problem (roadmap §10.4) |
> | Under viability threat (κ-step substrates) an endogenous sound gate eliminates steady-state lethal evaluations (0.0 vs 27.2 ungated; robust 12/12 with capability guard, worst gap −0.0072), certificate-preserving repair preserves population memory through death (Δ = +0.060, p = 0.0011), and one sound gate subsumes stacked empirical mechanisms (E×O interaction p = 0.0003/0.0045) | Holds (§9.6) | Scalar genes, single κ step, n = 20; the bounded-substrate null (autonomy AUC Δ = −0.04, p = 0.67), the softsat F2 invalidation, and the high-gain META overall-hypothesis failure are pre-registered and reported; mechanisms individually have priors (Gödel machine, safe-set projection, vicarious selection) — only the unified side-by-side death-cost measurement under an *evolving* sound certificate is claimed |
