...
- **The navigability advantage is EA-specific (§6).** A gradient-trained char-LM is indifferent to the gate (all gates reach the same CE, well below the EVO band), and a shuffled-corpus null ties at unigram. Practical upshot: for a gradient-trained LM, pick the verifier on soundness/coverage alone — gradient handles navigability.
